...
public class MockGoalRepository implements GoalRepository {
    ...
    public int prepend(Goal goal) {
        ...
        goal = new Goal(id, goal.content(), minSortOrder - 1, goal.completed(), goal.completionDate(), goal.pending(), goal.recurring(), goal.recurrenceType(), goal.context(), goal.startDate(), null, null, null, false);
        subject.setValue(goal);
        goals.add(subject);
        goalSubject.setValue(goals.stream().map(Subject::getValue).collect(Collectors.toList()));
        return id;
    ...
    public int append(Goal goal) {
        ...
        int id = goals.stream().map(Subject::getValue).mapToInt(Goal::id).max().orElse(0) + 1;
        goal = new Goal(id, goal.content(), maxSortOrder + 1, goal.completed(), goal.completionDate(), goal.pending(), goal.recurring(), goal.recurrenceType(), goal.context(), goal.startDate(), null, null, null, false);
        subject.setValue(goal);
        goals.add(subject);
        goalSubject.setValue(goals.stream().map(Subject::getValue).collect(Collectors.toList()));
        return id;
